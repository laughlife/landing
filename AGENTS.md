...
- 每一个阶段开始之前，都先判断本阶段是否适合使用 auto agents / 多 agents 并发推进；如果任务可拆分、可并行验证或可让多个 agents 分别探索实现方案，则每个阶段都应使用多 agents 并发来提升效率。
...
- 每一次小版本或阶段性改动完成后，都立即提交一次，并在提交信息中标记本阶段完成的工作；不要等所有任务全部完成后再集中提交。
- 不自动 `git push`。只有用户明确要求推送时才执行。
- 默认只负责提交，不负责推送；`push` 交给人类手工执行，除非用户明确要求代为推送。
