...
- 除非用户在当前任务中明确指定需要测试，否则只进行静态验收，不主动运行测试、Lint、类型检查、构建、开发服务器、浏览器自动化或其它动态验证。
- 默认静态验收包括：使用 `rg` 核对引用与旧值、审查相关源码和配置、查看 `git diff`，并运行 `git diff --check` 检查差异格式。
- 用户明确指定测试时，只运行其指定范围内的验证；如未指定具体范围，应先以最小必要范围执行，避免无关测试和环境写入。
- 最终回复必须清楚标明本阶段采用静态验收，以及未运行测试、构建或动态验证。
- 依赖未安装时，只说明需要 `pnpm install`；不要自行安装，也不要改用其它包管理器。

## 6. 提交与交付

- 每次完成文件修改后，默认只对本次任务相关文件执行 `git add` 和 `git commit`；如果用户明确说不要提交，则跳过。
- 每一次小版本或阶段性改动完成后，都立即提交一次，并在提交信息中标记本阶段完成的工作；不要等所有任务全部完成后再集中提交。
- 所有 Git 提交信息必须使用中文，清楚说明本阶段完成的功能和验证结果；禁止使用英文提交信息，也禁止使用“更新”“修改”“修复问题”等含糊描述。
- 提交信息建议使用 `类型：中文阶段说明，验证：中文验证结果` 的格式，例如 `功能：完成数据库模型与初始迁移，验证：迁移脚本检查通过`、`修复：解决后台登录会话校验问题，验证：登录接口自测通过`、`测试：补充产品接口权限测试，验证：权限测试通过`。
