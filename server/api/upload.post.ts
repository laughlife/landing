...
export default defineEventHandler(async (event) => {
  ...
  try {
    return await withApiErrorBoundary(event, async () => {
      ...
      if (detected.mime.startsWith('image/') && detected.mime !== 'image/gif') {
        ...
        const thumbnailPath = join(directory, `${baseName}-thumb.webp`)
        ...
        for (const targetWidth of [480, 960, 1600]) {
          if (width && width > targetWidth) {
            const variantPath = join(directory, `${baseName}-${targetWidth}.webp`)
            await sharp(processed).resize({ width: targetWidth, withoutEnlargement: true }).webp({ quality: 85 }).toFile(variantPath)
      ...
      const media = await prisma.$transaction(async (transaction) => {
        const created = await transaction.mediaFile.create({ data: { originalName: basename(upload.originalFilename || 'upload').slice(0, 512), storedName, relativePath, url: `/uploads/${relativePath}`, mimeType: detected.mime, extension: extname(storedName), size: processed.byteLength, width, height, checksum: createHash('sha256').update(processed).digest('hex'), category: detected.mime.startsWith('image/') ? 'IMAGE' : 'DOCUMENT', createdBy: actor.id } })
        await writeAudit(event, { adminUserId: actor.id, module: 'media', action: 'UPLOAD', targetType: 'MediaFile', targetId: created.id, summary: `上传文件 ${created.originalName}` }, transaction)
        return created
      })
      return success(media, '上传成功')
    })
  } catch (error) {
    const cleanup = await Promise.allSettled(generatedPaths.map(path => rm(path, { force: true })))
    if (cleanup.some(result => result.status === 'rejected')) {
      console.error('[upload-cleanup-error] 上传失败后的物理文件补偿不完整')
    }
    throw error
  } finally {
    await rm(upload.filepath, { force: true }).catch(() => undefined)
