...
export default defineCachedEventHandler(async () => {
  ...
  const [site, company, banners, services, categories, products, partners, articles] = await Promise.all([
    prisma.siteSetting.findUnique({ where: { id: 1 }, select: { siteName: true, siteUrl: true, siteTitle: true, siteKeywords: true, siteDescription: true, logo: true, favicon: true, fallbackImage: true, footerText: true, copyright: true, icpNumber: true, contactConfig: true } }),
